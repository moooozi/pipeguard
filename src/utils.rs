...
use windows::Win32::System::Threading::{OpenProcess, PROCESS_QUERY_INFORMATION, PROCESS_VM_READ, QueryFullProcessImageNameW, PROCESS_NAME_WIN32};
...
pub fn get_process_path(pid: u32) -> Result<String> {
    ...
    let process = unsafe {
        OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, false, pid)
            .map_err(|_| NamedPipeError::Io(std::io::Error::new(std::io::ErrorKind::PermissionDenied, "Cannot open process")))?
    };
    ...
    unsafe {
        QueryFullProcessImageNameW(process, PROCESS_NAME_WIN32, PWSTR(buffer.as_mut_ptr()), &mut length)
            .map_err(|_| NamedPipeError::Io(std::io::Error::new(std::io::ErrorKind::Other, "Failed to query process image name")))?;
    }
...
pub fn get_server_pid<H: AsRawHandle>(handle: &H) -> Result<u32> {
    ...
    unsafe {
        if GetNamedPipeServerProcessId(HANDLE(handle.as_raw_handle()), &mut server_pid).is_err() {
            return Err(NamedPipeError::Io(std::io::Error::new(std::io::ErrorKind::Other, "Failed to get server PID")));
        }
    }
    Ok(server_pid)
}
...
pub fn get_client_pid<H: AsRawHandle>(handle: &H) -> Result<u32> {
    ...
    unsafe {
        if GetNamedPipeClientProcessId(HANDLE(handle.as_raw_handle()), &mut client_pid).is_err() {
            return Err(NamedPipeError::Io(std::io::Error::new(std::io::ErrorKind::Other, "Failed to get client PID")));
        }
    }
    Ok(client_pid)
}
...
pub fn verify_same_path(other_pid: u32) -> Result<()> {
    ...
    let self_path = std::env::current_exe().unwrap().to_string_lossy().to_string();

    // Compare paths (case-insensitive on Windows)
    if !other_path.eq_ignore_ascii_case(&self_path) {
        return Err(NamedPipeError::Io(std::io::Error::new(std::io::ErrorKind::PermissionDenied, "Process path does not match")));
    }
